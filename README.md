...
- **API keys** are never in code and never in saved connection profiles. They come from two places: `.env` (recommended — never touched by the app), or the Settings panel, which persists whatever you enter to `settings.json` in the project root **in plaintext**, overriding `.env` for that provider. `settings.json` is gitignored and the file is written with owner-only permissions (`0600`) where the OS supports it, but it is not encrypted — anyone with filesystem access to the machine (or the container) can read it. If that's a concern, set keys via `.env` instead and leave the Settings fields blank.
